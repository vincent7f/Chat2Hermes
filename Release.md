...
- [x] A3 输入区可用性增强（发送状态反馈与提示优化）。实现 commit：`e27508e`
- [x] B4 设置页信息分区（连接配置、模型与鉴权、稳定性）。实现 commit：`d7b2acd`
- [x] B5 Profile 操作反馈补强（切换/删除后的明确提示）。实现 commit：`6695de8`；修订 commit：`983fd52`（删除后明确提示切换目标 profile）
- [x] B6 术语统一（刷新/继续接收/重发等口径）。实现 commit：`0a8aada`
